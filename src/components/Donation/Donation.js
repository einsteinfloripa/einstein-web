function Donation () {
    return (
        <>
            <div className="relative">
                <div className="max-w-7xl mx-auto sm:px-6 lg:px-8">
                    <div className="absolute inset-0">
                        <div className="absolute inset-0 bg-home-pattern" />
                        <div className="absolute inset-x-0 bottom-0">
                            <div className="flex justify-center lg:w-full">
                                {/*<img
                                    className="lg:w-auto w-5/6 pt-50 justify-center"
                                    src="/images/educacao-a-arma-mais-poderosa.png"
                                    alt="docente e aluno"
                                />*/}
                            </div>
                        </div>
                    </div>
                    <div className="relative px-4 py-16 sm:px-6 sm:py-24 lg:py-32 lg:px-8">
                        <h1 className="text-center font-extrabold tracking-tight">
                            <span className="block text-blue-900 lg:text-5xl text-2xl">
                                “A EDUCAÇÃO É A ARMA MAIS PODEROSA PARA MUDAR O MUNDO” - MANDELA, Nelson
                            </span>
                        </h1>
                    </div>
                </div>
            </div>
            <div className="relative">
                <div className="max-w-7xl mx-auto sm:px-6 lg:px-8">
                    <div className="absolute inset-0">
                        <div className="absolute inset-0 bg-blue-900" />
                    </div>
                    <div className="relative px-4 py-16 sm:px-6 sm:py-16 lg:py-20 lg:px-8">
                        <h2 className="text-center font-extrabold tracking-tight">
                            <span className="block text-white lg:text-4xl text-2xl">
                                AJUDE A TRANSFORMAR A EDUCAÇÃO: SEJA UM DOADOR!
                            </span>
                        </h2>
                    </div>
                </div>
            </div>
            {/*<div className="relative">
                <div className="max-w-7xl mx-auto sm:px-6 lg:px-8">
                    <div className="absolute inset-0">
                    <div className="absolute inset-0 bg-blue-50	" />
                    </div>
                    <div className="relative px-4 py-16 sm:px-6 sm:py-16 lg:py-20 lg:px-8">
                    <img
                        className="lg:w-auto w-3/8 pt-30"
                        src="/images/photo.png"
                        alt="docente e aluno"
                    />
                    </div>
                </div>
            </div>*/}       
            <div className="relative">
                <div className="max-w-7xl mx-auto sm:px-6 lg:px-8">
                    <div className="absolute inset-0">
                        <div className="absolute inset-0 bg-blue-50	" />
                    </div>
                    <div className="relative px-4 py-16 sm:px-6 sm:py-16 lg:py-20 lg:px-8">
                        <h2 className="text-center font-extrabold tracking-tight">
                            <span className="block text-blue-900 lg:text-4xl text-2xl">
                                POR QUE SER UM AMIGO DO EINSTEIN FLORIPA?
                            </span>
                        </h2>
                    </div>
                </div>
            </div>
            <div className="relative">
                <div className="max-w-7xl mx-auto sm:px-6 lg:px-8">
                    <div className="absolute inset-0">
                        <div className="absolute inset-0 bg-blue-900	" />
                    </div>
                    <div className="relative px-4 py-16 sm:px-6 sm:py-16 lg:py-20 lg:px-8">
                        <h2 className="font-extrabold tracking-tight">
                            <span className="text-blue-900 bg-white lg:text-3xl text-2xl p-2">
                                01 MUDAR A EDUCAÇÃO
                            </span>
                            <span className="block text-white lg:text-2xl text-2xl py-4">
                                Se tornando um Amigo do Einstein você estará investindo na educação de Santa Catarina. Assim, você nos ajuda a fornecer melhores condições para os nossos alunos e professores.
                            </span>
                        </h2>
                    </div>
                </div>
            </div>

            <div className="relative">
                <div className="max-w-7xl mx-auto sm:px-6 lg:px-8">
                    <div className="absolute inset-0">
                        <div className="absolute inset-0 bg-white	" />
                    </div>
                    <div className="relative px-4 py-16 sm:px-6 sm:py-16 lg:py-20 lg:px-8">
                        <h2 className="font-extrabold tracking-tight">
                            <span className="bg-blue-900 text-white lg:text-3xl text-2xl p-2">
                                02 DUPLO IMPACTO
                            </span>
                            <span className="block text-blue-900 lg:text-2xl text-2xl py-4">
                                O Einstein Floripa já impactou mais de 600 vidas! Aqui, além de impactar a vida dos nossos alunos, procuramos fazer o mesmo com a vida dos organizadores e docentes, oferecendo capacitação e desenvolvimento para estes universitários. É um aprendizado de aluno para aluno!
                            </span>
                        </h2>
                    </div>
                </div>
            </div>

            <div className="relative">
                <div className="max-w-7xl mx-auto sm:px-6 lg:px-8">
                    <div className="absolute inset-0">
                        <div className="absolute inset-0 bg-blue-900	" />
                    </div>
                    <div className="relative px-4 py-16 sm:px-6 sm:py-16 lg:py-20 lg:px-8">
                        <h2 className="font-extrabold tracking-tight">
                            <span className="text-blue-900 bg-white lg:text-3xl text-2xl p-2">
                                03 MUDAR VIDAS
                            </span>
                            <span className="block text-white lg:text-2xl text-2xl py-4">
                                Se tornando um Amigo do Einstein você ajudará a melhorar a vida de jovens em situação de vulnerabilidade econômica, ajudando-os a ingressar em uma universidade de qualidade.
                            </span>
                        </h2>
                    </div>
                </div>
            </div>

            <div className="relative">
                <div className="max-w-7xl mx-auto sm:px-6 lg:px-8">
                    <div className="absolute inset-0">
                        <div className="absolute inset-0 bg-white	" />
                    </div>
                    <div className="relative px-4 py-16 sm:px-6 sm:py-16 lg:py-20 lg:px-8">
                        <h2 className="font-extrabold tracking-tight">
                            <span className="bg-blue-900 text-white lg:text-3xl text-2xl p-2">
                                04 REALIZAR SONHOS
                            </span>
                            <span className="block text-blue-900 lg:text-2xl text-2xl py-4">
                                Cada um de nossos alunos sonha em se graduar na especialidade favorita e se tornar um profissional de destaque. Nós abraçamos esses sonhos e ajudamos a realizá-los!
                            </span>
                        </h2>
                    </div>
                </div>
            </div>

            <div className="relative">
                <div className="max-w-7xl mx-auto sm:px-6 lg:px-8">
                    <div className="absolute inset-0">
                        <div className="absolute inset-0 bg-blue-900	" />
                    </div>
                    <div className="relative px-4 py-16 sm:px-6 sm:py-16 lg:py-20 lg:px-8">
                        <h2 className="font-extrabold tracking-tight">
                            <span className="text-blue-900 bg-white lg:text-3xl text-2xl p-2">
                                05 RECEBER BENEFÍCIOS EXCLUSIVOS
                            </span>
                            <span className="block text-white lg:text-2xl text-2xl py-4">
                                Além de todo o impacto positivo, nossos Amigos do Einstein ainda têm acesso a benefícios exclusivos, como a newsletter mensal da instituição, para ficar por dentro de tudo que acontece no Einstein e também benefícios oferecidos pelos nossos parceiros.
                            </span>
                        </h2>
                    </div>
                </div>
            </div>

            <div className="relative">
                <div className="max-w-7xl mx-auto sm:px-6 lg:px-8">
                    <div className="absolute inset-0">
                        <div className="absolute inset-0	" />
                    </div>
                    <div className="relative px-4 py-16 sm:px-6 sm:py-16 lg:py-20 lg:px-8">
                        <h2 className="font-extrabold tracking-tight text-center">
                            <span className="block lg:text-3xl text-2xl text-blue-900">
                                BORA FAZER PARTE DO PROGRAMA AMIGOS DO EINSTEIN?
                            </span>
                            <span className="block lg:text-2xl text-2xl text-blue-900 mt-2">
                                CLIQUE ABAIXO NO PLANO QUE MAIS SE ENCAIXA COM VOCÊ!
                            </span>
                        </h2>
                    </div>
                </div>
            </div>

            <div className="relative">
                <div className="grid lg:grid-cols-5 md:grid-cols-3">
                    <div className="bg-blue-900 group">
                        <a
                            href="https://pag.ae/7VZFt-ama"
                            target="_blank"
                        >
                            <div className="relative px-4 py-16 sm:px-6 sm:py-16 lg:py-20 lg:px-8 text-center space-y-6">
                            <h2 className="font-extrabold tracking-tight">
                                <span className="block text-white text-1xl md:text-2xl lg:text-3xl xl:text-4xl group-hover:text-blue-50">
                                R$
                                <span className="text-6xl md:text-7xl lg:text-8xl xl:text-9xl group-hover:text-blue-50">
                                    5
                                </span>
                                </span>
                                <span className="block text-white text-1xl md:text-3xl lg:text-4xl xl:text-5xl group-hover:text-blue-50">
                                Mensais
                                </span>
                            </h2>
                            <h3 className="font-extrabold tracking-tight">
                                <span className="text-base text-white group-hover:text-blue-50">
                                    Você proporciona material didático para dois estudantes
                                    por um mês
                                </span>
                            </h3>
                            </div>
                        </a>
                    </div>
                    <div style={{ backgroundColor: "#2F6BA8" }} className="bg-blue-800 group">
                        <a
                            href="https://pag.ae/7VZFuGRNM"
                            target="_blank"
                        >
                            <div className="relative px-4 py-16 sm:px-6 sm:py-16 lg:py-20 lg:px-8 text-center	space-y-6">
                            <h2 className="font-extrabold tracking-tight">
                                <span className="block text-white text-1xl md:text-2xl lg:text-3xl xl:text-4xl">
                                R$
                                <span className="text-6xl md:text-7xl lg:text-8xl xl:text-9xl">
                                    15
                                </span>
                                </span>
                                <span className="block text-white text-1xl md:text-3xl lg:text-4xl xl:text-5xl">
                                Mensais
                                </span>
                            </h2>
                            <h3 className="font-extrabold tracking-tight">
                                <span className="text-base text-white">
                                Você fornece material de aula para todos os nossos
                                professores e monitores por uma semana
                                </span>
                            </h3>
                            </div>
                        </a>
                    </div>

                    <div style={{ backgroundColor: "#0185C3" }} className="bg-blue-700">
                        <a
                            href="https://pag.ae/7VZFwJYQp"
                            target="_blank"
                        >
                            <div className="relative px-4 py-16 sm:px-6 sm:py-16 lg:py-20 lg:px-8 text-center	space-y-6">
                            <h2 className="font-extrabold tracking-tight">
                                <span className="block text-white text-1xl md:text-2xl lg:text-3xl xl:text-4xl">
                                R$
                                <span className="text-6xl md:text-7xl lg:text-8xl xl:text-9xl">
                                    25
                                </span>
                                </span>
                                <span className="block text-white text-1xl md:text-3xl lg:text-4xl xl:text-5xl">
                                Mensais
                                </span>
                            </h2>
                            <h3 className="font-extrabold tracking-tight">
                                <span className="text-base text-white">
                                Você garante a participação de um estudante em todos os
                                nossos simulados ao longo do ano
                                </span>
                            </h3>
                            </div>
                        </a>
                    </div>

                    <div style={{ backgroundColor: "#47B2DF" }} className="bg-blue-600">
                        <a
                            href="https://pag.ae/7W7aAqCHJ"
                            target="_blank"
                        >
                            <div className="relative px-4 py-16 sm:px-6 sm:py-16 lg:py-20 lg:px-8 text-center	space-y-6">
                            <h2 className="font-extrabold tracking-tight">
                                <span className="block text-white text-1xl md:text-2xl lg:text-3xl xl:text-4xl">
                                R$
                                <span className="text-6xl md:text-7xl lg:text-8xl xl:text-9xl">
                                    60
                                </span>
                                </span>
                                <span className="block text-white text-1xl md:text-3xl lg:text-4xl xl:text-5xl">
                                Mensais
                                </span>
                            </h2>
                            <h3 className="font-extrabold tracking-tight">
                                <span className="text-base text-white">
                                Você possibilita um mês de estudo para três alunos
                                </span>
                            </h3>
                            </div>
                        </a>
                    </div>

                    <div style={{ backgroundColor: "#6DD2E0" }} className="bg-blue-500">
                        <a
                            href="https://pag.ae/7W6vM9J4m"
                            target="_blank"
                        >
                            <div className="relative px-4 py-16 sm:px-6 sm:py-16 lg:py-20 lg:px-8 text-center	space-y-6">
                            <h2 className="font-extrabold tracking-tight">
                                <span className="block text-white text-1xl md:text-2xl lg:text-3xl xl:text-4xl">
                                R$
                                <span className="text-6xl md:text-7xl lg:text-8xl xl:text-9xl">
                                    100
                                </span>
                                </span>
                                <span className="block text-white text-1xl md:text-3xl lg:text-4xl xl:text-5xl">
                                Mensais
                                </span>
                            </h2>
                            <h3 className="font-extrabold tracking-tight">
                                <span className="text-base text-white">
                                Você proporciona meio ano letivo de estudo para um aluno
                                com todas as despesas do projeto incluídas
                                </span>
                            </h3>
                            </div>
                        </a>
                    </div>
                </div>
            </div>

            <div className="relative">
                <div className="max-w-7xl mx-auto sm:px-6 lg:px-8">
                    <div className="absolute inset-0">
                        <div className="absolute inset-0 bg-blue-50" />
                    </div>
                    <div className="relative px-4 py-16 sm:px-6 sm:py-16 lg:py-20 lg:px-8 space-y-6">
                        <h2 className="font-extrabold tracking-tight">
                            <span className="block text-center lg:text-3xl text-2xl text-blue-900">
                            Para doar outros valores:
                            </span>
                        </h2>
                        <div className="relative">
                            <div className="grid md:grid-cols-2">
                                <div className="relative">
                                    <div className="grid grid-cols-2 space-x-4">
                                        <img
                                            className="w-60"
                                            src="/images/bb.png"
                                            alt="docente e aluno"
                                        />
                                        <div className="text-blue-900">
                                            <strong>Banco do Brasil</strong> <br />
                                            PROJETO EINSTEIN PRE-VEST
                                            <br />
                                            <strong>CNPJ:</strong> 23.637.930/0001-56
                                            <br /> <strong>Agência:</strong> 4550-0
                                            <br /> <strong>Conta:</strong> 8894-3
                                            <br /> Para PIX utilize o CNPJ
                                        </div>
                                    </div>
                                </div>
                                <div>
                                    <a
                                    href="https://app.picpay.com/user/projetoeinstein"
                                    target="_blank"
                                    >
                                    <img className="w-60 m-auto pt-8 md:pt-0" src="/images/picpay.png" />
                                    </a>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </>
    )
}

export default Donation